...
public class FlameParticleTrail {

	public static void run() {
		for (Map.Entry<Player, String> current : Main.trail.entrySet()) {
			if (current.getValue().equals(TrailsNames.FLAME_PARTICLE_NAME)) {
				Particle particle0 = new Particle(EnumParticle.FLAME, current.getKey().getLocation().add(0, 0.5f, 0), true, 0, 0, 0, 0, 1, 0);
				particle0.sendAll();
			}
		}
