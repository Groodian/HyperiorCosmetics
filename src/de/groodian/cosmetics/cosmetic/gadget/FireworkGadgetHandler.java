...
public class FireworkGadgetHandler extends GadgetHandler<GadgetCosmetic> {
    ...
    protected void onRightClickAndNoTimeout(PlayerInteractEvent e) {
        if (e.getClickedBlock() != null) {
            Firework firework = (Firework) e.getPlayer().getWorld().spawnEntity(e.getClickedBlock().getLocation().add(0, 1.5, 0), EntityType.FIREWORK);
            ...
            FireworkEffect fireworkEffect = FireworkEffect.builder().flicker(random.nextBoolean()).withColor(c1).withFade(c2).with(fireworkType).trail(random.nextBoolean()).build();
